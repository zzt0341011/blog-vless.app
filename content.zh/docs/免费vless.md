---

title: "免费 Clash、VLESS、VMess、Trojan 节点（每日更新）"
description: "每日更新免费 Clash、VLESS、VMess、Trojan、Shadowsocks 节点，支持 Clash Verge Rev、Mihomo、v2rayN、Shadowrocket、Stash、Hiddify 等客户端。无需注册，复制订阅即可使用。"
keywords:

- 免费Clash节点
- 免费VLESS节点
- 免费VMess节点
- 免费Trojan节点
- 免费Shadowsocks节点
- Clash订阅
- Clash Meta
- Mihomo
- v2rayN
- Shadowrocket
- 免费代理
weight: 5

---

# 免费 Clash、VLESS、VMess、Trojan 节点（每日更新）

正在寻找**免费 Clash 节点**或**免费 VLESS 节点**？

本页面每天都会更新一批免费的代理节点，适用于学习、测试和日常轻度使用，无需注册，复制订阅链接即可导入客户端。

支持目前主流代理客户端，包括：

* [Clash Verge Rev](https://clashvergerev.app/zh)
* [Mihomo](https://mihomoparty.pro/zh/)
* [FlClash](https://flclash.xyz/zh)
* [Shadowrocket](https://shadowrocket.ink/zh)
* [Stash](https://apps.apple.com/us/app/stash-rule-based-proxy/id1596063349)
* v2rayN
* v2rayNG
* [Hiddify Next](https://hiddify.me/zh)
* [sing-box](https://sing-box.info/zh)

所有免费节点均会定期更新，尽可能保证可用性。如果部分节点失效，请更新订阅或等待下一次更新。

---

# 🚀 免费订阅地址

## Clash Meta 订阅

```text
https://sod.psjch.xyz/yaml/c6db78eed0b2035309891151b5d322f9.yaml
```

推荐用于：

* Clash Verge Rev
* Mihomo
* FlClash

---

## V2Ray 通用订阅

```text
hysteria2://1427eb10-7529-4d38-a6a2-01b337868b37@au1.xiaoliyu.cyou:4433?sni=au1.xiaoliyu.cyou&insecure=1#%E2%9D%93Other_1%7C2.5MB%2Fs
hysteria2://22d891f5-ad85-4824-b0b8-e30cf3d37eb5@mg2.mangshe.xyz:2056?sni=mg2.mangshe.xyz&insecure=1#%F0%9F%87%A9%F0%9F%87%AADE_4%7C3.1MB%2Fs
hysteria2://22d891f5-ad85-4824-b0b8-e30cf3d37eb5@mg2.mangshe.xyz:2056?sni=127.0.0.1&insecure=1#%F0%9F%87%A9%F0%9F%87%AADE_5%7C2.9MB%2Fs
vless://f06c6039-a489-3d78-a676-ed43be6deca3@155.117.199.13:39211?encryption=none&flow=xtls-rprx-vision&security=reality&sni=cdnjs.cloudflare.com&fp=chrome&pbk=JrWOojSFwFA93YjsiQkDydTItQzNAsXsHU24HuWGWjA&sid=bc8e3658db7d1323&type=tcp&headerType=none#%F0%9F%87%B0%F0%9F%87%B7KR_1%7C1.9MB%2Fs
vless://4caaaae0-516b-4026-b2b2-69fea69df19a@5r86-tuq187.operix.top:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=www.bing.com&fp=chrome&pbk=LDpWYUJjGRJomvaN-gfgY96EJOF2DDHiFcNPEBLtoW0&sid=9019bbea8ff8781e&type=tcp&headerType=none#%F0%9F%87%B5%F0%9F%87%AAPE_2%7C1.4MB%2Fs
vless://f06c6039-a489-3d78-a676-ed43be6deca3@155.117.199.13:29893?encryption=none&flow=xtls-rprx-vision&security=reality&sni=cdnjs.cloudflare.com&fp=chrome&pbk=JrWOojSFwFA93YjsiQkDydTItQzNAsXsHU24HuWGWjA&sid=bc8e3658db7d1323&type=tcp&headerType=none#%F0%9F%87%AB%F0%9F%87%B7FR_2%7C2.7MB%2Fs
vless://055dce30-0ee3-44c3-b680-be1c95b8d6e7@107.151.250.68:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=www.bing.com&fp=firefox&pbk=g5UW6bf2soPSDNx7j0wanW_uLAtvTgQ8_lfztTJaP0k&sid=7025c954ff913b4c&type=tcp&headerType=none#%F0%9F%87%B5%F0%9F%87%AAPE_1%7C1.2MB%2Fs
vless://bb05f9d7-a3d1-4b81-a790-f6dba0301907@23.81.34.120:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=gb.servers.gracevpnbuy.com&fp=firefox&pbk=i00_tpIO0Oe2TV2lxm2BLm6QUzzkncyJtauMa7Rn500&sid=b91a73752053b039&type=tcp&headerType=none#%F0%9F%87%AC%F0%9F%87%A7GB_2%7C2.0MB%2Fs
vless://15e83ced-7544-422e-863d-deafe29a1dbe@81.90.21.59:8444?encryption=none&security=reality&sni=dl.google.com&fp=chrome&pbk=jgf0wWpgkEJZzu9wu45Ms33SNZpibtx507rpvzfz3mQ&sid=635a76c25a4b1820&type=xhttp&path=%2Fxh&mode=stream-one&extra=%7B%22xPaddingBytes%22%3A%22100-1000%22%7D#%F0%9F%87%B8%F0%9F%87%AASE_1%7C1.5MB%2Fs
vless://73bcd72f-9545-4cb8-8daf-7d004501880d@104.17.123.101:2083?encryption=none&security=tls&sni=wzknb.pages.dev&fp=chrome&allowInsecure=1&type=ws&host=wzknb.pages.dev&path=%2Fproxyip%3D167.253.153.215#%F0%9F%87%BA%F0%9F%87%B8US_4%7C2.6MB%2Fs
vless://b5ca5ab3-8cbc-4ac3-89da-9cd360589ecb@89.117.123.243:443?encryption=none&flow=xtls-rprx-vision&security=reality&sni=us.savesafe.cc&fp=firefox&pbk=zDeF_twyyT7a7zKiURF-Q8f6kkj95rx7zQ_jar09Hi4&sid=e4f6a3446ac34a23&type=tcp&headerType=none#%F0%9F%87%BA%F0%9F%87%B8US_14%7C1.3MB%2Fs

```

适用于：

* v2rayN
* v2rayNG
* Hiddify Next
* sing-box



---

## Base64 订阅

```text
https://vless.app/base64.txt
```

适用于支持 Base64 订阅格式的客户端。

---
# 支持的协议

免费订阅中可能包含以下协议：

* VLESS
* VMess
* Trojan
* Shadowsocks（SS）
* Reality
* TLS
* WebSocket（WS）
* gRPC
* HTTP/2

不同节点可能采用不同协议，请根据客户端兼容性选择使用。

---

# 为什么选择我们的免费节点？

相比网络上大量长期无人维护、失效严重的免费节点，我们会持续更新订阅，尽可能提供稳定、可用的测试节点。

我们的优势包括：

* ✅ 每日更新节点
* ✅ 支持 Clash Meta（Mihomo）
* ✅ 支持 IPv4 与 IPv6
* ✅ 覆盖多个国家和地区
* ✅ 无需注册即可使用
* ✅ 一键导入客户端
* ✅ 简单易用，适合新手

我们建议收藏本页面，以便第一时间获取最新的免费订阅链接。

# 📱 支持的客户端

无论您使用 Windows、macOS、Linux、Android 还是 iPhone，都可以通过以下客户端快速导入免费订阅。

| 平台            | 推荐客户端                        | 支持情况  |
| ------------- | ---------------------------- | ----- |
| Windows       | Clash Verge Rev、v2rayN       | ⭐⭐⭐⭐⭐ |
| macOS         | Clash Verge Rev、ClashX Meta  | ⭐⭐⭐⭐⭐ |
| Linux         | Clash Verge Rev、Mihomo       | ⭐⭐⭐⭐⭐ |
| Android       | [v2rayNG](https://v2rayng.4566.lol/zh)、Hiddify Next、FlClash | ⭐⭐⭐⭐⭐ |
| iPhone / iPad | Shadowrocket、Stash、Sing-Box  | ⭐⭐⭐⭐⭐ |

大多数客户端都支持直接导入订阅链接，只需复制订阅地址，粘贴到客户端即可完成配置，无需手动添加节点。

---

# ⚠️ 免费节点有哪些限制？

免费节点主要用于体验和测试，因此相比付费节点，会存在一定限制。

您可能会遇到以下情况：

* 网络高峰时速度下降
* 带宽有限
* 部分节点临时维护或失效
* 可选国家和地区较少
* 在线人数较多时延迟增加
* 不保证长期稳定在线

为了让更多用户能够免费体验，我们会定期清理失效节点，并持续补充新的免费服务器。

如果只是日常浏览网页、学习测试或偶尔使用，免费节点已经能够满足基本需求。

---

# 🚀 为什么推荐使用付费节点？

如果您有以下需求：

* 长时间稳定连接
* 高清视频播放（YouTube、Netflix 等）
* 在线办公
* 视频会议
* 游戏加速
* 大流量下载
* AI 工具（如 ChatGPT、Claude、Gemini）稳定访问

那么建议选择付费套餐。

相比免费节点，付费节点通常具有以下优势：

* 🚀 更高的连接速度
* 🌍 更多国家和地区节点
* 🔒 更高的稳定性和在线率
* 🎮 更低的网络延迟
* 📺 更好的流媒体解锁能力
* 🔄 自动更新订阅
* 📈 更大的流量配额
* 💬 专业客服支持
* 🛡️ 更少的拥堵，更好的使用体验

对于需要长期稳定使用的用户来说，付费节点通常能够节省大量时间和精力。

---

# ⭐ 推荐机场（高速稳定）

经过长期测试，我们推荐以下特点的机场服务：

* 全球多个节点
* 支持 VLESS、Trojan、Shadowsocks、Hysteria2 等协议
* 支持 Clash Meta、Mihomo、Shadowrocket、v2rayN 等客户端
* 提供高速专线和低延迟线路
* 支持自动订阅更新
* 支持多个设备同时在线
* 新用户购买更优惠（如有活动）

## 👉 点击购买付费节点

> **[网际快车机场，稳定，告诉，建立时间长，不限时间6.8元20G](https://1.jnk.ink/ad2RVl)：**

```text
https://1.jnk.ink/ad2RVl
```

如果您希望获得更稳定、更快速的网络体验，推荐选择付费套餐。大多数机场都提供多种套餐，可根据自己的使用需求灵活选择。

---

# 💡 免费节点还是付费节点？

| 对比项目    | 免费节点      | 付费节点          |
| ------- | --------- | ------------- |
| 使用费用    | 免费        | 需要订阅          |
| 更新频率    | 每日更新      | 实时维护          |
| 网络速度    | 一般        | 高速稳定          |
| 在线率     | 不保证       | 高可用           |
| 流媒体解锁   | 部分支持      | 更完善           |
| AI 工具访问 | 基本可用      | 更稳定           |
| 客服支持    | 无         | 有             |
| 推荐人群    | 新手体验、轻度使用 | 长期使用、办公、学习、娱乐 |

如果您是第一次接触代理工具，建议先使用免费节点体验；当您需要更好的速度和稳定性时，再升级到付费套餐即可。

# 📖 免费节点导入教程

本页面提供的免费订阅支持大多数主流代理客户端。

只需要复制订阅链接，然后按照下面教程操作，即可快速完成配置。

---

# Clash Verge Rev 导入订阅教程

[Clash Verge Rev](https://clashvergerev.app/zh) 是目前 Windows 和 macOS 用户常用的 Clash Meta 客户端。

## 操作步骤


![vless-clahsverge1001.jpg](https://vless.app/img/vless-clahsverge1001.jpg)

### 第一步：复制 Clash 订阅地址

复制本站提供的 Clash Meta 订阅链接：

```text
https://sod.psjch.xyz/yaml/c6db78eed0b2035309891151b5d322f9.yaml
```

---

### 第二步：打开 Clash Verge Rev

进入：

```
配置（Profiles）
```

点击：

```
新建订阅
```

或者：

```
Import Subscription
```

---

### 第三步：粘贴订阅链接

将复制的订阅地址粘贴进去：

```text
https://sod.psjch.xyz/yaml/c6db78eed0b2035309891151b5d322f9.yaml
```

点击：

```
导入
```

---

### 第四步：更新配置

![vless-clahsverge1002.jpg](https://vless.app/img/vless-clahsverge1002.jpg)

导入成功后：

1. 点击配置文件
2. 点击更新
3. 选择刚刚添加的配置
4. 开启系统代理

完成后即可使用。

---

# Shadowrocket 导入订阅教程（iPhone）


Shadowrocket 是 iPhone 用户常用的代理工具。

![vless-clahsverge1003.jpg](https://vless.app/img/vless-clahsverge1003.jpg)


## 操作步骤

### 第一步：复制订阅链接

复制：

```text
https://vless.app/base64.yaml
```

---

### 第二步：打开 Shadowrocket

点击右上角：

```
+
```

选择：

```
类型 → Subscribe
```

---

### 第三步：添加订阅

将订阅链接粘贴到 URL：

```text
https://your-domain.com/free/clash.yaml
```

点击：

```
完成
```

---

### 第四步：更新节点

返回首页：

点击：

```
更新订阅
```

等待节点加载完成。

选择需要使用的节点，然后开启连接即可。

---



# Hiddify Next 导入教程

[Hiddify Next](https://hiddify.me/zh) 支持 Android、Windows、macOS 和 Linux。

操作步骤：

1. 打开 Hiddify Next
2. 点击添加配置
3. 选择：

```
Add from URL
```

4. 粘贴订阅链接：


5. 点击确认
6. 更新节点

完成后即可连接。

---

# 常见导入问题

## 为什么导入后没有节点？

可能原因：

* 订阅地址复制错误
* 网络无法访问订阅服务器
* 客户端版本过旧
* 节点正在更新

解决方法：

1. 检查订阅地址
2. 更新客户端
3. 删除旧订阅重新添加
4. 更换网络环境再次尝试

---

## Clash Verge Rev 提示配置错误怎么办？

常见原因：

* 使用旧版 Clash 内核
* YAML 格式不兼容
* 订阅地址失效

建议：

* 使用最新版 Clash Verge Rev
* 使用 Mihomo 内核
* 重新更新订阅

---

## iPhone 为什么连接不上？

检查：

* 是否允许添加 VPN 配置
* 是否开启代理
* 是否选择有效节点
* 是否关闭其他 VPN 软件

---

# 下一步

如果免费节点无法满足您的速度和稳定性需求，可以升级到高速稳定套餐：

👉 查看推荐机场：

| 名 称 | 价 格 | 流 量 | 节点数 |
| :--- | :--- | :--- | :--- |
| [魔戒](https://1.jnk.ink/L4q20S) | 1元 | 1G | 30个 |
| [网际快车](https://1.jnk.ink/ad2RVl) | 7元 | 20G | 54个 |
| [牛逼](https://1.jnk.ink/LYet7x) | 14元 | 200G | 31个 |
| [飞鸟](https://1.jnk.ink/i7OhaC) | 10元 | 200G | 25个 |
| [皮卡丘](https://1.jnk.ink/d07dCA) | 15元 | 20G | 40个 |
| [happy猫](https://1.jnk.ink/5KiTxY) | 20元 | 200G | 27个 |
| [宝贝云](https://1.jnk.ink/xxPwfy) | 55元 | 600G | 64个 |
| [自由猫](https://1.jnk.ink/haO8Dr) | 89元 | 200G | 71个 |
| [飞兔](https://1.jnk.ink/bbXkiN) | 30元 | 100G | 80个 |

[点击打开网际快车机场](https://1.jnk.ink/ad2RVl)


- 使用邮箱，注册账号
- 你将获得：

* 更高速线路
* 更多节点选择
* 更稳定连接
* 更大流量额度


![vless-clahsverge1004.jpg](https://vless.app/img/vless-clahsverge1004.jpg)


- 注册成功以后
- 点击商店--找到--6.8元20g不限时间套餐

![vless-clahsverge1005.jpg](https://vless.app/img/vless-clahsverge1005.jpg)

- 点击使用教程--点击万能订阅--复制订阅

![vless-clahsverge1006.jpg](https://vless.app/img/vless-clahsverge1006.jpg)

- 导入到v2rayn、clashverge、sing-box、hiddify等

---

# 相关教程

* Clash Verge Rev 使用教程
* [Shadowrocket 完整使用指南](https://shadowrocket.ink/zh)
* v2rayN 配置教程
* VLESS Reality 节点教程
* [Mihomo 入门教程](https://mihomoparty.pro/zh/)
